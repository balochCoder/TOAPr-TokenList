...
namespace TOAPr1 {
  class Program {
    static void Main(string[] args) {
      ...
      string[] tokenlines = File.ReadAllLines(@"tokenList.txt");
      string curState = "";
      bool check = false;
      for (int j = 0; j < tokenlines.Length; j++) {
        Console.Write("Testing {0} ...", tokenlines[j]);
        char[] token = tokenlines[j].ToCharArray();
